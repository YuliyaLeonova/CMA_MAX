...
void HungarianAlgorithm::assignmentoptimal(int* assignment, double* cost, double* distMatrix, int nOfRows, int nOfColumns) {
    ...
    for (int row = 0; row < nOfRows; row++) {
        ...
        for (int col = 0; col < nOfColumns; col++) {
            maxValue = std::max(maxValue, distMatrix[col * nOfRows + row]); // Берём максимум
        }
        for (int col = 0; col < nOfColumns; col++) {
            distMatrix[col * nOfRows + row] -= maxValue; // Вычитаем максимум
        }
    }
...
#pragma omp parallel for
    for (int row = 0; row < nOfRows; ++row) {
        int col = assignment[row];
        if (col >= 0) {
            selectedValues[row] = originalDistMatrix[col * nOfRows + row];
        }
    }

    *cost = std::accumulate(selectedValues.begin(), selectedValues.end(), 0.0);  // Просто сумма
}*/
...
void HungarianAlgorithm::step3(int* assignment, double* distMatrix, char* starMatrix, char* newStarMatrix,
    ...
    while (zerosFound) {
        if (++iterationCount > MAX_ITERATIONS) {
            std::cerr << "ERROR: step3 exceeded maximum iterations!" << std::endl;
            return;
        }
...
void HungarianAlgorithm::step5(int* assignment, double* distMatrix, char* starMatrix, char* newStarMatrix,
    char* primeMatrix, char* coveredColumns, char* coveredRows, int nOfRows, int nOfColumns, int minDim) {
    double h = std::numeric_limits<double>::lowest();  // Теперь ищем максимум
    int chunk_size = 64;

#pragma omp parallel for reduction(max : h) schedule(dynamic, chunk_size)
    for (int row = 0; row < nOfRows; row++) {
        if (!coveredRows[row]) {
            for (int col = 0; col < nOfColumns; col++) {
                if (!coveredColumns[col]) {
                    h = std::max(h, distMatrix[col * nOfRows + row]);
                }
            }
        }
    }
    ...
    if (h == std::numeric_limits<double>::lowest()) {
        std::cerr << "ERROR: No uncovered elements found in step5!" << std::endl;
        return;
    }
